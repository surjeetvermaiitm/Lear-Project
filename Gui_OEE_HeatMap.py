...
def calc_duration_parameters(st, et, l = None):
    ...
    if(len(dt_relevant)>0):
        ...
        availability = 1 - unplanned_dt/dt.timedelta(hours = no_hrs)
...
def main():
    ...
    ch_bt1 = Button(fr1, image = cfim, command = clicked1)
    ch_bt1.photo = cfim
...
def window2():
    ...
    back_bt = Button(fr2, image = bim, command = bkclick) 
    back_bt.photo = bim
    ...
    time_bt = Button(fr2, image = ptim, command = clicked2)
    time_bt.photo = ptim
...
def window3():
    ...
    def inplot():
        ...
        for widget in frame1_1.winfo_children():
            widget.destroy()
        hm = ttk.Button(frame1_1, image = him, command = plotinmap)
        hm.photo = him
        RC = ttk.Button(frame1_1, image = rcim, command = lambda : RunChartParameters(window3, 1))
        RC.photo = rcim
    ...
    def begin(l = None):
        global flag, res, res_rd, plot_flag, p_table
        ...
        begin_A = Button(frame2, image = lA, command = lambda: clicked3('A'))
        begin_A.photo = lA
        begin_B = Button(frame2, image = lB, command = lambda: clicked3('B'))
        begin_B.photo = lB
        begin_rep= Button(frame2, text = 'Generate complete Report', style = 'W.TButton', command = gen_pdf)
        params = ttk.Button(frame2, image = proq, command = printout)
        params.photo = proq
        hm = ttk.Button(frame2, image = him, command = lambda : plotmap(p_table))
        hm.photo = him
        RC = ttk.Button(frame2, image = rcim, command = lambda : RunChartParameters(window3,0,l))
        RC.photo = rcim
        ip = ttk.Button(frame2, image = ipim, command = inplot)
        ip.photo = ipim
