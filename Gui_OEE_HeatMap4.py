...
class table:
    ...
    def listform(self, root, lst, buttonflag):
        ...
        if buttonflag == 'True':
            ...
            self.b5 = Button(self.fr1, width = 12, text = 'No GO', style = 'W.TButton', command = NGCodes)
    ...
    def tableform(self, root, lst, ngf):
         self.rows = len(lst)
         self.columns = len(lst[0])
         for i in range(self.rows):
            self.fr = Frame(root)
            ...
            for j in range(self.columns):
                if i in range(3):
                    if ngf == 1:
                        self.e = Label(self.fr, width = 20, text = str(lst[i][j]), font = ('Arial Bold',18), relief = RAISED, foreground = 'black', background= '#76f685', borderwidth = 1)
                    else:
    ...
    def __init__(self, root, lst, buttonflag = 'False', testflag = 0, ngf = 0):
        if testflag == 0 :
            self.listform(root, lst, buttonflag)
        if testflag == 1:
            self.tableform(root, lst, ngf)
...
def NGCodes():
    ...
    for i in range(len(ngc)):
        if re.search(r"NG",str(ngc[i])):
            lst.append(ngc[i])
    
    lst_ngc = Counter(lst).most_common()
    ngwin = Tk()
    table(ngwin, lst_ngc, testflag = 1, ngf = 1)
    ngwin.mainloop()
